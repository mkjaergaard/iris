...
{
  iris::glog<iris::Fatal>("Assertion Failed",
                 ...
                 int line)
{
  ((condition)
   ...
   : iris::assert_failed(func, file, line));
}
                 ...
                 const char* name1, const arg<T1>& argument1)
{
  ((condition)
   ? __ASSERT_VOID_CAST(0)
   : iris::assert_failed(func, file, line, name1, argument1));
}

}

#define BEAM_ASSERT(c)                                                  \
  iris::test_assert((c), BOOST_CURRENT_FUNCTION, __FILE__, __LINE__)

#define BEAM_ASSERT_1(x, a1, v1)                                        \
  iris::test_assert((x), BOOST_CURRENT_FUNCTION, __FILE__, __LINE__, a1, v1)
